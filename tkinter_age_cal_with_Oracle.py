...
def databasee():
    con = cx_Oracle.connect("system/bULLI@localhost/orcl")
    cur = con.cursor()
    tq = "create table Age_cal(sno int primary key, day int, month int, year int, Given_day int, Given_month int, Given_year int, RYear int, Rmonth int, Rday int)"
    cur.execute(tq)
    cur.close()
    con.close()

def storeAgeInDatabase(sno, day, month, year, gday, gmonth, gyear, rday, rmonth, ryear):
    try:
        con = cx_Oracle.connect("system/bULLI@localhost/orcl")
        cur = con.cursor()

        iq = "insert into Age_cal values(:sno, :day, :month, :year, :gday, :gmonth, :gyear, :rday, :rmonth, :ryear)"
        cur.execute(iq, (sno, day, month, year, gday, gmonth, gyear, rday, rmonth, ryear))

        con.commit()
        messagebox.showinfo("Success", "Age stored in Oracle Database successfully!")

    except cx_Oracle.Error as error:
        messagebox.showerror("Error", f"Error storing age in Oracle Database: {error}")
...
def calculateAge():
    ...
    if value == -1:
        ...
        # Incrementing sno value automatically
        con = cx_Oracle.connect("system/bULLI@localhost/orcl")
        cur = con.cursor()
        cur.execute("SELECT MAX(sno) FROM Age_cal")
        result = cur.fetchone()
        sno = 1 if result[0] is None else result[0] + 1
        cur.close()
        con.close()
